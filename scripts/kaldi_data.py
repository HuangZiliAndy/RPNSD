...
# - extract_segmemts(data_dir)
#       returns segmented audio data as numpy array.
...
def load_spk2idx(spk2idx_file):
    """ returns dictionary { recid: duration }  """
    if not os.path.exists(spk2idx_file):
        return None
    lines = [line.strip().split(None, 1) for line in open(spk2idx_file)]
...
if __name__ == '__main__':
    for utt, data in extract_segments(sys.argv[1]):
        print(utt, data)
