...
for fold_num in {1..5}; do
  exp_dir=experiment/pretrain${pretrain_expname}${pretrain_modelname}freeze${freeze}bnfix${set_bn_fix}cfg${cfg}epoch${epochs}bs${batch_size}op${optimizer}lr${lr}min_lr${min_lr}pat${patience}seed${seed}alpha${alpha}arch${arch}ls${large_scale}/$fold_num
  ...
  CUDA_VISIBLE_DEVICES=`free-gpu -n 1` scripts/train.py $exp_dir $train_dir $dev_dir --cfg_file $cfg_file \
	  --freeze $freeze --set_bn_fix $set_bn_fix --pretrain_model $pretrain_model \
  	  ...
  	  --arch $arch --large_scale $large_scale --alpha $alpha \
